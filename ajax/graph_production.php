...
if ($_REQUEST['t'] == '5')	{  // Consumi accumulati edificio
			...
			foreach($usages as $usage)	{
				for($year=$y1;$year<=$y2;$year++)	{
					...
					foreach($flats as $flat)	{
						...
						$consumo1 += $flat1->get_npvm2_primary($usage['ID_USAGE'], $y1, 1);
						$consumo2 += $flat1->get_npvm2_primary($usage['ID_USAGE'], $y1, 2);
					}	
...
if ($_REQUEST['t'] == '7')	{  // Consumi totali appartamento 
	...
	$usages = array(array(1));
	
	$colori = sfumature(2, 1);
	...
	// Per ogni misuratore creo un array contenente tutti i consumi periodici
		...
		$bplot[$j]->SetLegend("Energia termina - riscaldamento acqua");
		$j++;
