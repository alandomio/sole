...
# CONFIGURAZIONE NW
$scheda->ext_table(array('gruppis'));

# VISUALIZZAZIONE LISTA
$strcut = 30;

$aStripC = array();
$aShowList = array('USER','NAME','SURNAME');
$aShowCrud = array_diff($scheda -> aFields, $aStripC);

# CONFIGURAZIONE FILTRI
$my_vars = new ordinamento(array('us' => 'USER', 'nm' => 'NAME', 'sn' => 'SURNAME', 'iu' => 'IS_UPLOADER', 'ig' => 'GRUPPI'));
$my_vars -> set_filtro(array('vaiu' => '1', 'slig' => '5')); # SOLO GLI UPLOADER

$my_vars->campi_force['text']['USER'] = 'users';  //FORZO IL FORMATO DEL CAMPO (FIELDNAME_EM) A SELECT (select) PASSANDO IL NOME TABELLA (tablename) RELATIVO
...
$scheda->etichetta = 'Contatori'; // 'se non specificata qui, � il nome tabella senza la s finale'
...
$href_rem_search = io::a($MYFILE -> file, array_merge($backUri, $vars_back, array('id' => $id, 'rem_search' => '1')), REM_ALL_REC, array('class' => 'g-button g-button-red'));

$puls_aggiorna = '<input name="upd" type="submit" value="'.SAVE.'" class="g-button g-button-yellow">';
...
foreach($sub_nav as $k => $arr){
	...
	elseif($crud=='ins') { $sub_menu.='<div class="menu_att_dis">'.$k.'</div>'; }
		...
		$href_to = io::a($arr[0], array_merge($backUri, $addCrud), $k, array('title' => strtolower($k)));
...
<table class="list">
<tr class="bg">  
<th colspan="4"><?=$href_lista?> <?=$puls_aggiorna?></th>
</tr>
<tr class="yellow"><td colspan="<?=$my_vars->colonne+1?>"><?=$etichetta?></td></tr>
