...
$html['flat-info'] = '';
if(!empty($_GET['id'])){
	$id_building = prepare4sql($_GET['id']);
}
...
$province = '';
if(!empty($r['K1_ID_PROVINCE'])){
	$province = '('.$r['K1_ID_PROVINCE'].')';
}
...
'.$r['DESCRIP_BLD'].'
'.$r['ADDRESS_BLD'].' '.$r['K1_ID_COMUNI'].$province.'
</div>
...
$html['hcompany'] = '<ul style="list-style:none; padding:0;">';
foreach($a as $k => $f){
	if(!empty($r[$f])){
		$html['hcompany'] .= '<li><strong>'.constant($f).':</strong><br />'.$r[$f].'</li>'."\n";
	}
}
