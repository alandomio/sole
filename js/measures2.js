$(document).ready(function(){
	$('#federations').change(function(){
	 ...
	 $('#delete-ms').click(function(){
			var year = $('#year').val();
			var uploadtype = $('#uploadtype').val() + 1;
	 ...
	 $('#uploadtype').change(function(){
		 	var year = $('#year').val();
			var uploadtype = parseInt($('#uploadtype').val());
			var buildings = $('#buildings').val();

			if(year && uploadtype && buildings){
				$('#del-measures').removeClass('hidden');
				var answer = confirm('Confermi la cancellazione delle convalide?');
				if (answer){
					// elimino le misurazioni correlate
					
					// http://sole.dev/ajax/json.php?action=delete_all_ms&building=1&year=2012&upload_type=2
					
					
					jQuery.getJSON("ajax/json.php?action=delete_all_ms&building=" + buildings + '&year='+year + '&upload_type=' + uploadtype,
					  function(data){
							if(data.success){
			...
			$('#uploadtype option[value=""]').attr('selected','selected');
	 });
	...
	$('#print-measures').click(function(){
		...
		if( id == '' ){
			...
			window.open( 'print_measures12.php?id=' + id + '&year=' + year + '&uploadtype=' + uploadtype );
			return false;
		}
	});*/
...
function print_page(){
	...
	if( id == '' ){
		...
		window.open( 'print_measures2.php?id=' + id + '&year=' + year );
		return false;
	}
}
...
function delete_rows(){
	$('.dele_measures').each(function(){
		...
		if( $( this ).is(':checked') ){
			...
			var element = $( this );
			$.getJSON("ajax/json.php?action=del_measure2&year=" + values[0] + '&id_uploadtype=' + values[1] + '&id_meter=' + values[2],
			function( data ){
				if(data.success){
...
$("#dialog-download").dialog({
	...
	buttons: {
		'Download' : function(){
			window.open( 'ajax/excel.php?action=modello&tipo=semestrale&anno='+$('#year').val()+'&mese='+ $('#month_download').val() +'&id_building=' + $('#buildings').val() );
...
function showMEASURES(){
	...
	if(id_building && year){
		...
		$('#table-content').load('ajax/json.php?action=measures_2_form&id=' + id_building + '&year=' + year, function(){
...
function save_measure(year, id_uploadtype, id_meter, fieldname, value, campo){
	jQuery.getJSON('ajax/json.php?action=save_measure2&year=' + year + '&id_uploadtype=' + id_uploadtype + '&id_meter=' + id_meter+ '&fieldname=' + fieldname+ '&value=' + value,
	function(data){
		if(data.success){
