...
if($scheda -> img){ # STAMPO CONTROLLI IMMAGINE
	...
	print'<tr><td>'.UPD_INS_IMG.'</td><td colspan=\"5\"><input name="nw_img" type="file" /></td></tr>';
	if(is_file($scheda->img_main_web.$path[$scheda->f_path])){
		$href_deleimg = io::a($scheda->file_c, array_merge($backUri,array('nw_img_dele' => $id, 'crud'=>'upd')), DELE_IMG, array('title' => CANCEL));
	
		//$href_deleimg = io::href($scheda->file_c, $val=array_merge($backUri,array('nw_img_dele' => $id, 'crud'=>'upd')), $txt=DELE_IMG,$js='',$target="",$title=CANCEL, $id='', $css="");
		print"<tr><td>$href_deleimg</td><td colspan=\"5\">
