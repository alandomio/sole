...
if(!empty($_GET['mknew'])){
	$newid = $_GET['mknew'];
	$qc = "SELECT * FROM myfiles WHERE ID_MYFILE = '$newid'";
	$rc = rs::rec2arr($qc);
	if(empty($rc['ID_MYFILE'])){
		$q = "INSERT INTO myfiles (ID_MYFILE) VALUES ('$newid')";
		mysql_query($q);
	}
...
foreach($sublable as $k=>$v){
	...
	?>
<tr>
<td valign="top" width="160"><?=$v?></td><td colspan="5"><? $db->$k->get(); ?></td>
</tr><? }}?>
